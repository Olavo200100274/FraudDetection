...
Class Weights is, on balance, the safest strategy across model families: it preserves or improves baseline PR-AUC for the FT-Transformer and the gradient-boosted models on BAF, and only occasionally degrades performance substantially (the main exception being LGBM on ULB, which drops by roughly 17\% under Class Weights, in line with its sensitivity to every non-baseline strategy on that dataset).
