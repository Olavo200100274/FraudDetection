...
\end{tikzpicture}
\captionof{figure}{End-to-end experimental pipeline for fraud detection under extreme class imbalance. Stages~3--5 operate exclusively on the DEV partition (80\%) via inner cross-validation; stages~6--7 provide a single unbiased evaluation on the held-out TEST set (20\%); stage~8 adds post-hoc explainability via SHAP.}
\label{fig:pipeline}
...
\end{tikzpicture}
\captionof{figure}{Leakage-free inner cross-validation protocol. Within each fold, preprocessing is fitted and resampling is applied exclusively on the training subset (left); the validation subset (right) is only transformed and scored. The dashed red line represents the leakage barrier: no information may cross from right to left. The final decision threshold is the median across all $K{=}5$ folds.}
