...
Tuning uses \textbf{Optuna}~\cite{akiba2019optuna} with the Tree-structured Parzen Estimator (TPE) sampler and a \texttt{MedianPruner} that terminates unpromising trials early (based on per-fold intermediate results). Each study runs 50~trials (configurable), optimising PR-AUC via 5-fold stratified cross-validation. For gradient-boosted models (LightGBM and CatBoost), early stopping (patience~=~50 rounds) determines the optimal number of boosting iterations, avoiding both over-fitting and wasteful computation. TPE is seeded for full reproducibility.
