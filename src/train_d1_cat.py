...
from src.models.cat import build_model
...
def main():
    ...
    # Preprocess OUTSIDE (avoid sklearn Pipeline incompat)
    with timed("Preprocessing (fit on train, transform all)", timings):
        pre = build_preprocessor_d1(log_amount=True)
        X_train_p = pre.fit_transform(X_train, y_train)
        X_val_p = pre.transform(X_val)
        X_test_p = pre.transform(X_test)

    with timed("Train CatBoost (fit)", timings):
        model = build_model(SEED)
        model.fit(X_train_p, y_train)

    with timed("Predictions + threshold sweep (validation)", timings):
        val_score = model.predict_proba(X_val_p)[:, 1]
    ...
    with timed("Save CSV artifacts", timings):
        sweep_df.to_csv(os.path.join(RESULTS_DIR_D1, "threshold_sweep_CAT.csv"), index=False)
        metrics.to_csv(os.path.join(RESULTS_DIR_D1, "metrics_CAT.csv"), index=False)
        cm_df.to_csv(os.path.join(RESULTS_DIR_D1, "confusion_matrix_tauF2_CAT.csv"), index=True)
        pr_df.to_csv(os.path.join(RESULTS_DIR_D1, "pr_curve_CAT.csv"), index=False)
    ...
    with timed("Generate plots", timings):
        ...
        plt.ylabel("Score")
        plt.title("D1 (ULB 2013) — CatBoost Threshold Sweep (Validation)")
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(FIGURES_DIR_D1, "threshold_sweep_CAT.png"), dpi=200)
        plt.close()
        ...
        plt.title("D1 (ULB 2013) — CatBoost Precision-Recall Curve (Test)")
        plt.tight_layout()
        plt.savefig(os.path.join(FIGURES_DIR_D1, "pr_curve_CAT.png"), dpi=200)
        plt.close()
    ...
    with timed("Save timing report", timings):
        ...
        timing_df.to_csv(os.path.join(RESULTS_DIR_D1, "timing_CAT.csv"), index=False)
