...
from src.models.rf import build_model
...
def main():
    ...
    with timed("Save CSV artifacts", timings):
        sweep_df.to_csv(os.path.join(RESULTS_DIR_D1, "threshold_sweep_RF.csv"), index=False)
        metrics.to_csv(os.path.join(RESULTS_DIR_D1, "metrics_RF.csv"), index=False)
        cm_df.to_csv(os.path.join(RESULTS_DIR_D1, "confusion_matrix_tauF2_RF.csv"), index=True)
        pr_df.to_csv(os.path.join(RESULTS_DIR_D1, "pr_curve_RF.csv"), index=False)
    ...
    with timed("Generate plots", timings):
        ...
        plt.ylabel("Score")
        plt.title("D1 (ULB 2013) — RF Threshold Sweep (Validation)")
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(FIGURES_DIR_D1, "threshold_sweep_RF.png"), dpi=200)
        plt.close()
        ...
        plt.title("D1 (ULB 2013) — RF Precision-Recall Curve (Test)")
        plt.tight_layout()
        plt.savefig(os.path.join(FIGURES_DIR_D1, "pr_curve_RF.png"), dpi=200)
        plt.close()
    ...
    with timed("Save timing report", timings):
        ...
        timing_df.to_csv(os.path.join(RESULTS_DIR_D1, "timing_RF.csv"), index=False)
